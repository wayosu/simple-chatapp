        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) { // if email is valid
            ...
            if (mysqli_num_rows($sql) > 0) { // if email already exist
                ...
                if (isset($_FILES['image'])) { // if file is uploaded
                    ...
                    $tmp_name = $_FILES['image']['tmp_name']; // this temporary name is used to saved/move file in our folder
                    ...
                    if (in_array($img_ext, $extensions) === true) { // if user uploaded img ext is mathced with any array extensions
                        $time = time(); // this will return us current time
                                        // we need this time because when you uploading user img to in our folder we rename user file
                        ...
                        if (move_uploaded_file($tmp_name, "images/" . $new_img_name)) { // if user upload img move to our folder successfully
                            ...
                            $sql2 = $conn->query("INSERT INTO users (unique_id, fname, lname, email, password, img, status) 
